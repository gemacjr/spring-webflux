...
public class ProductApiAnnotationApplication {
	...
	public CommandLineRunner init(ReactiveMongoOperations operations, ProductRepository productRepository) {
		return args -> {
			Flux<Product> flux = Flux.just(new Product(null, "Big Latte", 2.99), new Product(null, "Big Decaf", 2.49),
					new Product(null, "Green team", 1.99)).flatMap(productRepository::save);

			flux.thenMany(productRepository.findAll()).subscribe(System.out::println);
